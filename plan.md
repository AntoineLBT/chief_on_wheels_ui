...
- [ ] Create `src/stores/auth.ts` — Pinia store holding `accessToken`, `refreshToken` (persisted to localStorage), `login()`, `logout()`, `isAuthenticated` getter
- [ ] Create `LoginPage.vue` — email + password form, calls store `login()`, redirects to home on success
- [ ] Add router guard: unauthenticated users redirected to `/login`

## 3. Dexie schema
- [ ] Create `src/services/db.ts` — Dexie subclass with version 1 schema:
